...
if (occasionBtn) {
  occasionBtn.addEventListener("click", async () => {
    try {
      ...
      occasionResult.innerText = "Asking AI stylist for an outfit...";
...
if (pairBtn) {
  pairBtn.addEventListener("click", async () => {
    try {
      const base = baseItemInput.value.trim();
      if (!base) {
        alert("Describe the piece you are definitely wearing (e.g. 'black t-shirt').");
        return;
      }

      pairResult.innerText = "Asking AI how to complete your look...";

      // Load wardrobe from Firestore
      const wardrobe = await loadWardrobe();

      if (wardrobe.length === 0) {
        pairResult.innerText = "Your wardrobe is empty. Add items first.";
        return;
      }
      ...
      if (!response.ok) {
        const errorData = await response.json();
        pairResult.innerText = `Error: ${errorData.error}`;
        return;
      }

      const data = await response.json();
      pairResult.innerText = data.pairing;
    } catch (err) {
      console.error(err);
      pairResult.innerText = "AI stylist had an error. Check console.";
    }
  });
}
